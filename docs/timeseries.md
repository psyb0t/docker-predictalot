...
- **moirai-2** — fixed 9-quantile output `{0.1..0.9}`; we filter to your requested subset. The wrapper context comes from `PREDICTALOT_MOIRAI_MAX_CONTEXT`. Wrappers are cached per requested horizon up to `PREDICTALOT_MOIRAI_MAX_HORIZON`, and shorter inputs are zero-padded with a `past_is_pad` mask. Multivariate requests are limited to the model's native 64-step window and return `400` above it.
